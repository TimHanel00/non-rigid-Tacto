...
class TactoController(Sofa.Core.Controller):
    ...
    def getDofForce(self):
        ...
        if len(self.forceBuf) == 4:
            ...
            return np.sum(np.array(
                self.forceBuf))  # use the last three force messurements to smooth out the data delivered to tacto
    ...
    def changePos(self):
        ...
        if self.key == '=':
            self.transformWrapper.setPosition(
                [t[0] + self.XYZ[0] * self.scale, t[1] + self.XYZ[1] * self.scale, t[2] + self.XYZ[2] * self.scale])
        if self.key == '-':
    ...
    def getAngles(self):
        orientation_list = self.transformWrapper.getOrientation().tolist()
        eulerAngles = Quat(orientation_list).getEulerAngles()
    ...
    def createNewSphere(self):
        ...
        MO = newSphere.addObject('MechanicalObject', position=[0, 1.5 + 0.5 * self.iteration, 0, 0, 0, 0, 1],
                                 name=f'Particle-{self.iteration}', template='Rigid3d')
        ...
        Sphere = newSphere.addObject('SphereCollisionModel', name="SCM", simulated=1, moving=1, radius=0.02,
    ...
    def __init__(self, name: str, meshfile: str, parent: Sofa.Core.Node, tissue, stiffness=5.0, senderD=None,
                 forceMode: ForceMode = ForceMode.dof, controllMode: ControllMode = ControllMode.position):
        ...
        # self.rigidobject = self.node.addObject("MechanicalObject", template="Rigid3d", name="TactoMechanics",
        #                                        position=[0.0, 0.13, 0, 0, 0, -0.7071068, 0.7071068])



        # No position parameter passed, indicating the use of the default or existing positions
        self.rigidobject = self.node.addObject(
            "MechanicalObject",
            template="Rigid3d",
            name="TactoMechanics"
        )
        ...
        # Create GelScene and bind it to the rigid body object of the TactoController
        self.gel_scene = GelScene(vtk_file="mesh/gel_tet_4.2.vtk", tacto_mechanics=self.rigidobject)
        ...
        self.mode = 0
        self.forceDict = {ForceMode.dof: self.getDofForce, ForceMode.normal: self.getNormalForce,
                          ForceMode.vertex: self.getCollisionEstimatedForce}
